...
async def get_stats(current_user: Dict = Depends(require_admin)):
    ...
    # 各種統計情報を取得
    stats = {
        "total_users": supabase_client.table("users")
        .select("id", count="exact")
        .execute()
        .count,
        "total_posts": supabase_client.table("posts")
        .select("id", count="exact")
        .execute()
        .count,
        "total_shrines": supabase_client.table("shrines")
        .select("id", count="exact")
        .execute()
        .count,
        "total_goshuin": supabase_client.table("goshuin_types")
        .select("id", count="exact")
        .execute()
        .count,
    }
...
async def update_user_role(
    ...
    update_response = (
        ...
        .update({"role_id": request.new_role_id})
        .eq("id", user_id)
        .execute()
    )
...
async def get_pending_approvals(current_user: Dict = Depends(require_moderator)):
    ...
    # 承認待ちの御朱印
    pending_goshuin = (
        supabase_client.table("goshuin_types")
        .select("id, name, shrine_id, created_at, created_by_user_id")
        .eq("status", "pending")
        .execute()
    )
...
@router.post(
    "/approve/{resource_type}/{resource_id}",
    dependencies=[Depends(require_permission("approve_content"))],
)
